...
if (isset($_SERVER['SCRIPT_FILENAME']) && basename($_SERVER['SCRIPT_FILENAME']) === 'comments.php') {
    die(esc_html__('Accesso diretto non consentito.', 'novi'));
}

if (post_password_required()) { ?>
    <p class="no-comments alignwide">
        <?php esc_html_e('Questo articolo è protetto da password. Inseriscila per vedere i commenti.', 'novi'); ?></p>
<?php return;
}
...
if (have_comments()) : ?>
    <section id="comments" class="comments alignwide">
        <h3 id="comments-title">
            <?php
            printf(
                _n('%s Commento', '%s Commenti', get_comments_number(), 'novi'),
        ...
        <nav class="comment-navigation">
            <div class="nav-previous">
                <?php previous_comments_link('&larr; ' . esc_html__('Commenti precedenti', 'novi')); ?></div>
            <div class="nav-next"><?php next_comments_link(esc_html__('Commenti successivi', 'novi') . ' &rarr;'); ?>
            </div>
        </nav>
        ...
        <?php if (!comments_open()) : ?>
            <p class="no-comments"><?php esc_html_e('I commenti sono chiusi.', 'novi'); ?></p>
        <?php endif; ?>
    </section>
<?php endif; ?>
...
<section class="comment-respond">
    <?php
    $comment_form_args = [
        'title_reply'          => esc_html__('Lascia un commento', 'novi'),
        'title_reply_before'   => '<h3 id="reply-title" class="comment-reply-title">',
        'title_reply_after'    => '</h3>',
        'comment_notes_before' => '',
        'comment_notes_after'  => '',
        'label_submit'         => esc_html__('Invia commento', 'novi'),
        'cancel_reply_link'    => esc_html__('Annulla risposta', 'novi'),
        'fields'               => [
            'author' => '<p class="comment-form-author">
                    <label for="author">' . esc_html__('Nome', 'novi') . ' <span class="required">*</span></label>
        ...
        'comment_field'        => '<p class="comment-form-comment">
                                <label for="comment">' . esc_html__('Commento', 'novi') . '</label>
